...
Where C stands for the contributing area of a given stream, 𝛃 stands for the coefficient term of stream magnitude (M), and 𝜖 stands for an error term (i.e. unexplained variation in C).
...
ggplot() + 
  ...
  ggthemes::theme_map() + 
  labs(title = '*T. tubifex* predicted density based on stream slope and contributing area',
       subtitle = 'Adjusted for Inorganic Carbon Percentage, where data available',
       fill = 'T. tubifex/m^2^') + 
  theme(legend.position = 'right',
        legend.title = ggtext::element_markdown(),
        plot.title = ggtext::element_markdown())
...
leaflet() |>
  ...
  addLegend(
    title = '<em>T. tubifex</em><br>predicted<br> density<br>(indivs/m<sup>2</sup>)',
    pal = leaflet::colorNumeric(palette = 'Spectral',
                                    domain = str_d_wgs$tubifex_density_m2),
    values = str_d_wgs$tubifex_density_m2,
...
# library(ggiraph)
# gg_point = ggplot(str_d_wgs[c(1:100),]) + 
#   geom_sf_interactive(
#     aes(
#       col = tubifex_density_m2,
#       tooltip = tubifex_density_m2,
#       data_id = tubifex_density_m2
#     )
#   )
# 
